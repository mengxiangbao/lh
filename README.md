...
```text
data/raw/daily_price_long_clean.csv
```
...
启动界面时，侧边栏会自动填入配置中的日线路径（默认 `data/raw/daily_price_long_clean.csv`）以及 `data` 目录下的输出目录和数据检查目录。你也可以手动改成 Z 盘路径。
...
最低需要一张点时化日线表，可以是 CSV 或 Parquet。项目默认示例路径是：
